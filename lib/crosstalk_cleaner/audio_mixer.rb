...
module CrosstalkCleaner
  ...
  #
  # The gate is a single +volume+ filter driven by a timeline +enable+
  # expression, so each track is processed in one pass at ffmpeg's native frame
  # size. That is what keeps the mix fast even with thousands of owned blocks.
  class AudioMixer
