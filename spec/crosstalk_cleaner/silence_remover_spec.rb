...
RSpec.describe CrosstalkCleaner::SilenceRemover do
  ...
  describe "#silence_filter" do
    it "keeps at most the given amount of silence and declicks the splices" do
      expect(remover.silence_filter(0.75))
        .to eq("silenceremove=stop_periods=-1:stop_duration=0.750:stop_threshold=-30dB,adeclick")
    end
    ...
    it "uses the configured noise floor as the threshold" do
      ...
      expect(quiet.silence_filter(0.75))
        .to eq("silenceremove=stop_periods=-1:stop_duration=0.750:stop_threshold=-50dB,adeclick")
    end
